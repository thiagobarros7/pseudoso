class Memoria:
    ...
    def aloca_processo(self, offset, tamanho_processo, PID):
        for index in range(offset, tamanho_processo+offset):
            self.memoria[index] = PID

    def aloca_processo_usuario(self, tamanho_processo, PID):
        if self.verifica_memoria(self.offset_usuario, tamanho_processo):
            self.aloca_processo(self.offset_usuario, tamanho_processo, PID)
    ...
    def aloca_processo_nucleo(self, tamanho_processo, PID):
        if self.verifica_memoria(self.offset_nucleo, tamanho_processo, 1):
            self.aloca_processo(self.offset_nucleo, tamanho_processo, PID)
